...
export default function ProductsTable({ initialProducts, categories }) {
  ...
  if (products.length === 0) {
    return (
      <div className="rounded-2xl bg-white p-10 text-center text-brand-deep-900/50 shadow-sm ring-1 ring-brand-blush-100">
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        {products.map((p) => (
          <div key={p.id} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-brand-blush-100">
            <div className="flex gap-3">
              <div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-brand-blush-50">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.images?.[0]} alt="" className="h-full w-full object-cover" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate font-semibold text-brand-deep-900">{p.name}</p>
                <p className="text-xs text-brand-deep-900/50">{categoryName(p.category)}</p>
                <div className="mt-1 flex items-center gap-2">
                  <span className="font-bold text-brand-plum-700">{money(p.price)}</span>
                  <span className="text-xs text-brand-deep-900/50">· Stock {p.stock}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 flex gap-2 border-t border-brand-blush-50 pt-3">
              <Link
                href={`/admin/productos/${p.id}`}
                className="flex-1 rounded-full border border-brand-plum-600/30 py-2 text-center text-xs font-semibold text-brand-plum-700 hover:bg-brand-blush-50"
              >
                Editar
              </Link>
              <button
                disabled={busyId === p.id}
                onClick={() => remove(p)}
                className="flex-1 rounded-full border border-red-200 py-2 text-xs font-semibold text-red-600 hover:bg-red-50"
              >
                Eliminar
              </button>
            </div>
      ...
      <div className="hidden overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-brand-blush-100 sm:block">
              ...
              <tr key={p.id} className="border-b border-brand-blush-50 last:border-0">
                ...
                <td className="px-4 py-3">
                  <button
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-semibold ${
                      p.featured
                        ? "bg-brand-orchid-500 text-white"
                        : "bg-brand-blush-100 text-brand-deep-900/60"
                    }`}
                  >
                    {p.featured ? "Sí" : "No"}
                  </button>
